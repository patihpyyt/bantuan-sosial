<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Sesuaikan informasi distribusi finansial wilayah yang telah tercatat.</p>
            </div>
            <div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="p-6 bg-slate-50 border-b border-slate-100">
                <h3 class="text-sm font-bold text-slate-700 uppercase tracking-wider">Form Perubahan Data Distribusi</h3>
            </div>

            <form action="{{ route('provinsi.distribusi.update', $distribusi->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="kabupaten_id" id="kabupaten_id" 
                            ...
                            @foreach($kabupatenList as $kab)
                                <option value="{{ $kab->id }}" {{ old('kabupaten_id', $distribusi->kabupaten_id) == $kab->id ? 'selected' : '' }}>
                                    {{ $kab->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input type="text" id="jumlah_display" inputmode="numeric" autocomplete="off" placeholder="Contoh: 50.000.000"
                                ...
                                value="{{ old('jumlah', $distribusi->jumlah) ? number_format(old('jumlah', $distribusi->jumlah), 0, ',', '.') : '' }}" required>
                        </div>
                        <input type="hidden" name="jumlah" id="jumlah" value="{{ old('jumlah', $distribusi->jumlah) }}">
                        <p class="mt-1.5 text-[11px] font-medium text-slate-400">Pastikan nominal digit angka nol sudah benar sebelum disimpan.</p>
                        @error('jumlah')
                            <p class="mt-1.5 text-xs font-semibold text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Format mask untuk data lama bawaan database saat halaman pertama dimuat
            if (display.value) {
                let cleanValue = display.value.replace(/\D/g, '');
