<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">Ubah data distribusi alokasi anggaran yang sudah tercatat.</p>
            </div>
            <div>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <form action="{{ route('anggaran.update', $anggaran->id) }}" method="POST" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select name="kabupaten_id" id="kabupaten_id" 
                            ...
                            @foreach($kabupaten as $kab)
                                <option value="{{ $kab->id }}" {{ old('kabupaten_id', $anggaran->kabupaten_id) == $kab->id ? 'selected' : '' }}>
                                    {{ $kab->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input type="text" id="total_anggaran_display" inputmode="numeric" autocomplete="off" placeholder="Contoh: 234.000.000"
                                ...
                                value="{{ old('total_anggaran', $anggaran->total_anggaran) ? number_format(old('total_anggaran', $anggaran->total_anggaran), 0, ',', '.') : '' }}" required>
                        </div>
                        <input type="hidden" name="total_anggaran" id="total_anggaran" value="{{ old('total_anggaran', $anggaran->total_anggaran) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Format otomatis untuk nilai yang sudah ada dari database saat load halaman
            if (display.value) {
